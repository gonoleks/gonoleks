...
func TestContext_BindXML(t *testing.T) {
	...
	// Set up XML request
	requestCtx.Request.Header.SetContentType(MIMEApplicationXML)
	requestCtx.Request.SetBody([]byte(`<TestUser><name>john</name><email>john@example.com</email></TestUser>`))
...
func TestContext_BindQuery(t *testing.T) {
	...
	// Set up query parameters
	requestCtx.QueryArgs().Add("name", "john")
	requestCtx.QueryArgs().Add("email", "john@example.com")
...
func TestContext_BindUri(t *testing.T) {
	...
	// Set up URI parameters
	ctx.AddParam("name", "john")
